...
struct `Audio Tests` {
    ...
    @Test
    func `Audio with disable remote playback`() {
    ...
    @Test
    func `Complete audio configuration`() {
        let src = Src("audio-file.mp3")
        let controls = Controls()
        let autoplay = Autoplay()
        let loop = Loop()
