...
extension `Performance Tests` {
    ...
    struct ByteSerialization {
        ...
        @Test("Autofocus: RFC pattern", .timed(threshold: .microseconds(10)))
        func autofocusRFCPattern() {
            let attr = Autofocus(value: true)
            let _ = [UInt8](attr)
        }
        
        @Test("Autofocus: UTF-8 direct", .timed(threshold: .microseconds(10)))
        ...
        @Test("Id ASCII: RFC pattern", .timed(threshold: .microseconds(50)))
        func idASCIIRFCPattern() {
            let attr = Id(value: "main-content")
            let _ = [UInt8].init(attr)
        }
        
        @Test("Id ASCII: String interpolation", .timed(threshold: .microseconds(50)))
        ...
        @Test("Id UTF-8: RFC pattern", .timed(threshold: .microseconds(100)))
        func idUTF8RFCPattern() {
            let attr = Id(value: "日本語-コンテンツ")
            let _ = [UInt8](attr)
        }
        
        @Test("Id UTF-8: String interpolation", .timed(threshold: .microseconds(100)))
        ...
        @Test("Batch RFC: 100 attributes", .timed(threshold: .milliseconds(5), maxAllocations: 100_000))
        ...
        @Test("Batch String: 100 attributes", .timed(threshold: .milliseconds(5), maxAllocations: 100_000))
        ...
        @Test("Direct bytes only", .timed(threshold: .microseconds(50)))
        func directBytesOnly() {
            let attr = Id(value: "test")
            let _ = [UInt8](attr)
        }
        
        @Test("Bytes via String composition", .timed(threshold: .microseconds(100)))
        ...
        @Test("HTML generation RFC: 10 attributes", .timed(threshold: .milliseconds(1), maxAllocations: 50_000))
