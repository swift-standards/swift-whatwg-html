...
@Suite
struct `WHATWG_HTML_GlobalAttributes.Title Test` {
    @Test
    func `WHATWG_HTML_GlobalAttributes.Title attribute should be title`() {
        #expect(WHATWG_HTML_GlobalAttributes.Title.attribute == "title")
    }

    @Test
    func `WHATWG_HTML_GlobalAttributes.Title should store and return its value`() {
        let title = WHATWG_HTML_GlobalAttributes.Title("Helpful information")
        #expect(title.rawValue == "Helpful information")
    }

    @Test
    func `WHATWG_HTML_GlobalAttributes.Title description should return its value`() {
        let title = WHATWG_HTML_GlobalAttributes.Title("Click to submit the form")
        #expect(title.description == "Click to submit the form")
    }

    @Test
    func `WHATWG_HTML_GlobalAttributes.Title should be initializable with string literal`() {
